...
export function ThreadBulkActionBar({ archiveMailboxId, disabled = false, onAction, selectedCount }: ThreadBulkActionBarProps) {
  if (selectedCount <= 0) {
    return null;
  }
  ...
  return (
    <div aria-live="polite" className="rounded-[20px] border border-accent/25 bg-accent/8 px-4 py-3" data-testid="thread-bulk-bar">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        ...
        <MailActionStrip availability={{ archive: Boolean(archiveMailboxId) }} disabled={disabled} onAction={onAction} />
      </div>
    </div>
  );
}
